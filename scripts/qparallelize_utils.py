...
def parmap(function, argsList, default_return=None, threads=1, chunksize=500):
  ...
  inQueue  = Queue(1);
  outQueue = Queue();
  subprocesses = [Process(target=pExecFunction, args=(function, inQueue, outQueue, default_return)) for _ in xrange(threads)];
  for proc in subprocesses:
    proc.daemon = True;
    proc.start();
  curJobCount = 0;
  while True:
    newArgs = [inQueue.put((idx, args)) for idx, args in izip(islice(idxList, chunksize), islice(argsList, chunksize))];
...
def parstarmap(function, argsList, default_return=None, threads=1, chunksize=500):
  ...
  inQueue  = Queue(1);
  outQueue = Queue();
  subprocesses = [Process(target=pExecStarFunction, args=(function, inQueue, outQueue, default_return)) for _ in xrange(threads)];
  for proc in subprocesses:
    proc.daemon = True;
    proc.start();
  curJobCount = 0;
...
def paristarmap(function, argsList, default_return=None, threads=1, chunksize=500):
  ...
  inQueue  = Queue(1);
  outQueue = Queue();
  subprocesses = [Process(target=pExecStarFunction, args=(function, inQueue, outQueue)) for _ in xrange(threads)];
  for proc in subprocesses:
    proc.daemon = True;
    proc.start();
  curJobCount = 0;
