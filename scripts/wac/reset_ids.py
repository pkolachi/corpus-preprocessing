...
DEF = 2 ;
INFILES = '' if len(sys.argv) < DEF    else sys.argv[1:-1] ; 
...
def main_loop() :
  ...
  for inf in INFILES : 
    with ru.smart_open(inf, large=True, fast=True) as infile : 
      for ln in infile : 
        ln = ln.strip() ;
        try : 
          ln = ln.decode('utf-8') ;  
        ...
        if ln == '# newpar' : 
          ...
          docm  = '# newdoc id = {0}'.format(docpath+fname) ; 
          yield docm ; 
          pcount = 0 ; 
          scount = 0 ; 
          dcount += 1 ; 
...
import gzip ; 
with gzip.GzipFile(OUTFILE, mode='wb') as outfile :
  for line in main_loop() :
    s = line+'\n';
